...
function changeUpsellOutboundQuantity(quant) {
    ...
    var addQuantToField = parseInt(quant);
    var calcPrice = "";
    ...
    if (quant == 'n') {
        ...
        if (quant == 'd') {
            ...
            addQuantToField = parseInt($('#additionalNum').val());
            $('#hasPost').val("0");
        } else {
            if (quant == "1"){
    ...
    $('.firstPriceBox').show();
    $('.priceField').empty();
    $('.priceField').append('Total price: '+calcPrice);
...
//*********************************************************************
function confirmUpsellOutbound(){

        var narucenaCijena  = $('#price').val();

        if (parseInt($('#quantity').val()) > 1) {
            narucenaCijena = parseInt($('#price').val()) + (parseInt($('#upsell').val()) * (parseInt($('#quantity').val())-1) );
        }

        var podaci = {};
        podaci["action"]    = "changeOMGorder";
        ...
        podaci["houseno"]   = $('#_houseno').val();
        podaci["city"]      = $('#_city').val();
        podaci["postal"]    = $('#_postal').val();
        podaci["telephone"] = $('#_telephone').val();
        podaci["mail"]      = $('#_mail').val();

        podaci["quantity"]  = $('#quantity').val();
        podaci["price"]     = $('#price').val();
        podaci["upsell"]    = $('#upsell').val();
        podaci["postage"]   = $('#hasPost').val();
        podaci["newPrice"]  = $('#calcPrice').val();

        podaci["upsellChangeMade"]   = $('#upsellChangeMade').val();
        podaci["dataChangeMade"]     = $('#dataChangeMade').val();

        podaci["comment"]   = $('#comment').val();
