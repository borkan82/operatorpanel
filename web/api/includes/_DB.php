...
class Database
{
	...
	public function query($sql, $type=1, $cache=false)
	{
		...
		session_start();
		if (isset($_SESSION['infomedia_sesion_id']) && !empty($_SESSION['infomedia_sesion_id']) && $_SESSION['infomedia_sesion_id'] != 0 && is_numeric($_SESSION['infomedia_sesion_id'])){
			$timetocheck = Date('Y-m-d H:i:s');
			$sqlcheck = "";

			$this->_query = mysql_query($sqlcheck, $this->_connect);
		}
		...
		if ($queryTime > 10){
			...
			if(ROOT){
				...
				$logFile = fopen(ROOT ."/logs/dbslow/". date('d-m-Y') .".txt", "a");
				try{
				...
				if(file_exists(ROOT. "/logs/dbslow/". date('d-m-Y') .".txt")){

					chmod(ROOT. "/logs/dbslow/". date('d-m-Y') .".txt", 0777);
				}
			}
		}
